...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		// Eagerly check singleton cache for manually registered singletons. //todo 从一级缓存获得 找的就用 找不到就返回null 准备创建
		...
		if (sharedInstance != null && args == null) {
			...
			// We're assumably within a circular reference. //TODO 如果不是单例就抛异常
			if (isPrototypeCurrentlyInCreation(beanName)) {
				throw new BeanCurrentlyInCreationException(beanName);
			}

			// Check if bean definition exists in this factory.  检查有没有父类
			...
			try {
				...
				}//spring中的object 所有bean都叫RootBeanDefinition
				RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.//todo 检查是否依赖其他bean
				...
				// Create bean instance.//TODO 如果是单例 创建bean实例。
				if (mbd.isSingleton()) {
					sharedInstance = getSingleton(beanName, () -> {//再次从一级缓存获取bean
						try {
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			...
			return beanInstance;
		}//todo 现在我们有了bean实例，它可能是一个普通的bean或一个FactoryBean。
		// 如果它是一个FactoryBean，我们将使用它来创建一个bean实例，除非调用者实际上需要一个对工厂的引用。
